...
module.exports = function() {
  return function(deck) {
    var ACTIVE = 'active', VIMEO_RE = /\/\/player\.vimeo\.com\//, YOUTUBE_RE = /\/\/www\.youtube\.com\/embed\//,
      ...
      findGifs = query.bind(null, 'img[src*=".gif"]'),
      ...
      play = function(obj) {
        var vol = obj.hasAttribute('data-volume') ? Math.max(Math.min(parseFloat(obj.getAttribute('data-volume')), 10), 0) : NaN;
        if (typeof obj.play === 'function') {
          ...
          if (YOUTUBE_RE.test(obj.src)) {
            if (!isNaN(vol)) post(obj, {event: 'command', func: 'setVolume', args: [vol*10]});
            post(obj, {event: 'command', func: 'playVideo'});
          }
          else if (VIMEO_RE.test(obj.src)) {
            if (file) return console.warn('WARNING: Cannot autoplay Vimeo video since deck is loaded from a file:// URI.');
      ...
      pause = function(obj) {
        if (typeof obj.pause === 'function') {
          ...
          if (YOUTUBE_RE.test(obj.src)) post(obj, {event: 'command', func: 'pauseVideo'});
      ...
      activateSvg = function(obj) {
        if (obj.getAttribute('data-reload') === 'true') reloadSvg(obj);
          ...
          if (svg.tagName === 'svg') svg.classList.add(ACTIVE);
            ...
            obj.addEventListener('load', function addActive() { // wait for the SVG to load into object container
