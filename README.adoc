...
* Built-in HTML audio (`<audio>` element)
* Built-in HTML video (`<video>` element)
* Vimeo video
* YouTube video
* Animated SVG (embedded using the `<object>` element)
* Animated GIF (image ending in `.gif`)
...
In addition to those attributes, this plugin recognizes the following custom attributes:

data-rewind _(type: boolean, default: false)_::
If this attribute is present, the audio stream is restarted each time playback is triggered.

data-volume _(type: integer)_::
This attribute can be used to set the volume of the audio stream.
The value must be an integer between 0 and 10, where 0 is muted and 10 is full volume.
Any value outside this range will be coerced to the closest range limit.
If this attribute is not specified, the volume of the audio stream is not modified.

NOTE: Using the `autoplay` and `volume` attributes is not recommended since they conflict with the behavior of this plugin.
...
data-rewind _(type: boolean, default: false)_::
If this attribute is present, the video is restarted each time playback is triggered.

data-volume _(type: integer)_::
This attribute can be used to set the volume of the video.
The value must be an integer between 0 and 10, where 0 is muted and 10 is full volume.
Any value outside this range will be coerced to the closest range limit.
If this attribute is not specified, the volume of the video is not modified.

NOTE: Using the `autoplay` attribute is not recommended since it conflicts with the behavior of this plugin.
...
This plugin also recognizes the following custom attributes:
...
This plugin recognizes the following custom attributes:

data-reload _(type: boolean, default: false)_::
If present, instructs the plugin to reload the SVG by reassigning the `data` attribute.
If this attribute is not present, the plugin toggles the CSS class on the root element of the SVG.
...
data-reload _(type: boolean, default: false)_::
If present, instructs the plugin to reload the GIF by reassigning the `src` attribute.
If this attribute is not present, the plugin takes no action on the GIF.
